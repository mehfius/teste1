...
echo "Iniciando batch scraper do Airbnb (Deno)"
echo "Configurações:"
...
DENO_OUTPUT=$("$DENO_CMD" run --allow-net --allow-env get_rooms.ts)
if [ $? -ne 0 ]; then
...
for ROOM_ID in $ROOM_IDS; do
  ...
  # Sempre extrair e atualizar o Supabase
  ACTION="extract-supabase"
  
  # Executar o scraper, forçando o uso do Deno em vez do puppeteer
  ./run_scraper.sh "$ROOM_ID" "$OUTPUT_DIR" "deno" "$ACTION"
...
# Criar um script Deno temporário para registrar o log
cat > register_log.ts << EOL
// Script temporário para registrar log de execução
...
async function main() {
  try {
    ...
    const roomsIdsStr = "${ALL_ROOMS_ARRAY}";
    const roomsIds = roomsIdsStr.split(',')
      .filter(id => id.trim())
      .map(id => parseInt(id.trim()));

    console.log(`Registrando log para \${roomsIds.length} quartos processados`);
    const result = await updater.logExecution(
      ${EXECUTION_TIME},
      roomsIds,
      ${SUCCESS},
      ${TOTAL_ROOMS}
    ...
    if (result.success) {
      ...
      console.error(`❌ Falha ao registrar log: \${result.error || 'Erro desconhecido'}`);
      Deno.exit(1);
    }
  } catch (error) {
...
# Executar o script
"$DENO_CMD" run --allow-net --allow-env register_log.ts
LOG_RESULT=$?
